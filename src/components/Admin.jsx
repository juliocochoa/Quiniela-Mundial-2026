...
export default function Admin() {
  ...
  const [systemLogs, setSystemLogs] = useState([
    {
      time: "19:42:01",
      msg: "System initialized. Listening on port 3000.",
    },
    { time: "19:42:15", msg: "Supabase client loaded successfully." },
    {
      time: "19:43:55",
      msg: "Admin authentication approved. Terminal active.",
    },
  ]);
    ...
    return dbMatches.map((m) => {
      ...
      return {
        ...
        status:
          override.status ??
          m.status ??
          (m.home_score != null ? "FINISHED" : "SCHEDULED"),
  ...
  const filters = useMemo(() => {
    ...
    return [
      ...
      ...dateFilters
        ...
        .map((d) => ({
          id: d,
          label: formatDateLabel(new Date(d + "T12:00:00")),
        })),
    ];
  }, [dbMatches, yesterday]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSystemStats((prev) => ({
        cpu: Math.max(
          12,
          Math.min(95, prev.cpu + Math.floor(Math.random() * 11) - 5),
        ),
        ram: Math.max(
          4.2,
          Math.min(
            7.9,
            parseFloat((prev.ram + Math.random() * 0.4 - 0.2).toFixed(2)),
          ),
        ),
        dbStatus: "ONLINE",
        responseTime: Math.max(
          ...
          Math.min(
            150,
            prev.responseTime + Math.floor(Math.random() * 15) - 7,
          ),
        ),
      }));
    }, 4000);
  ...
  const handleSaveMatch = async (match) => {
    ...
    if (override.homeScore !== undefined)
      updates.home_score = override.homeScore;
    if (override.awayScore !== undefined)
      updates.away_score = override.awayScore;
    ...
    addLog(
      `[MATCH_UPDATE] ${match.homeTeam} ${match.homeScore} - ${match.awayScore} ${match.awayTeam} (Status: ${match.status})`,
    );
    triggerToast(
      `Partido ${match.homeTeam} vs ${match.awayTeam} guardado.`,
    );
  };
  ...
  return (
    <div className="admin-terminal-root grid-bg min-h-screen relative overflow-hidden select-none">
      ...
      {/* Main Content Area */}
      <main className="p-8 min-h-screen pb-20">
        {/* MATCHES TAB */}
        {activeTab === "matches" && (
          <div className="animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-8">
              <div>
                <h2 className="text-4xl font-headline font-black text-white tracking-tighter uppercase">
                  Resultados de los Partidos (Admin)
                </h2>
              </div>
              <div className="flex gap-4">
                <div className="terminal-border bg-primary-container/20 p-4 flex flex-col min-w-[160px]">
                  <span className="text-[10px] font-mono text-[#7389ca] uppercase tracking-wider">
                    Completed
                  </span>
                  <span className="text-2xl font-bold text-white mt-1 font-headline">
            ...
            <div className="flex gap-3 mb-10 overflow-x-auto pb-2">
              {filters.map((f) => (
                <button
                  ...
                  className={`px-6 py-2 font-mono text-xs uppercase tracking-widest transition-all whitespace-nowrap ${
                    activeFilter === f.id
                      ? "terminal-tab-active"
                      : "terminal-tab hover:bg-primary-container/40"
                  }`}
                >
                  {f.label}
                </button>
                ...
                {filteredMatches.map((match) => (
                  <div
                    ...
                    className={`terminal-border bg-primary-container/10 overflow-hidden relative group transition-all duration-300 ${
                      match.status === "LIVE"
                        ? "shadow-[0_0_15px_rgba(115,137,202,0.2)]"
                        : ""
                    }`}
                  >
                    {/* Card Header */}
                    <div
                      className={`px-4 py-1.5 flex justify-between items-center ${
                        match.status === "LIVE"
                          ? "bg-secondary"
                          : "bg-[#7389ca]/20 border-b border-[#7389ca]/30"
                      }`}
                    >
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between gap-4">
                        ...
                        <div className="flex flex-col items-center flex-1">
                          ...
                          <span className="font-bold text-xs uppercase tracking-widest text-white truncate max-w-[80px]">
                            {match.homeTeam}
                          </span>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-[#7389ca] font-light text-xl">
                              :
                            </span>
                            <input
                              type="number"
                          ...
                          {match.status === "LIVE" ? (
                            <div className="flex items-center gap-1">
                              <span className="text-[9px] font-mono text-[#7389ca]">
                                TIME:
                              </span>
                              <input
                                type="text"
                            ...
                            <span className="text-[9px] font-mono text-[#7389ca] uppercase tracking-widest">
                              {match.status}
                            </span>
                          )}
                        </div>
        ...
        {activeTab === "system" && (
          <div className="animate-in fade-in duration-300 space-y-8">
            <div>
              <span className="font-mono text-xs text-secondary uppercase tracking-[0.3em] mb-2 block glow-text">
                SYS_TELEMETRY
              </span>
              <h2 className="text-4xl font-headline font-black text-white tracking-tighter uppercase">
                System Telemetry
              ...
              <p className="text-[#7389ca] mt-2 max-w-xl text-xs font-mono">
                [Telemetry stream online]. Tracking active CPU load, system
                RAM limits, database status, and Supabase service response
                time.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              ...
              <div className="terminal-border bg-primary-container/20 p-6 flex flex-col justify-between h-40">
                <div className="flex justify-between items-start">
                  <span className="text-xs font-mono text-[#7389ca] uppercase">
                    CPU Load
                  </span>
                  <span className="material-symbols-outlined text-[#7389ca]">
                    monitoring
                  </span>
                </div>
                <div>
              ...
              <div className="terminal-border bg-primary-container/20 p-6 flex flex-col justify-between h-40">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#7389ca] font-mono mt-2">
                    CONNECTED TO SERVER : US-EAST
                  </p>
                </div>
              </div>
            ...
            <section className="terminal-border bg-primary-container/10 p-6">
              <h3 className="text-lg font-headline font-bold text-white uppercase mb-4">
              ...
              <div className="font-mono text-xs text-[#7389ca] space-y-2 h-48 overflow-y-auto bg-[#000c2e] p-4 border border-[#7389ca]/30">
                ...
                <p>
                  &gt; SSL transport layer: TLS_AES_256_GCM_SHA384
                  (Verified)
                </p>
                <p>&gt; Cache eviction policy: LRU. Buffer hits: 98.4%</p>
                <p>
                  &gt; Server clock drift: -0.002s (synchronized NTP pool)
                </p>
                <p className="text-green-400">
                  &gt; Diagnostics completed. All systems operational.
                </p>
              </div>
            </section>
          </div>
        ...
        {activeTab === "users" && (
          <div className="animate-in fade-in duration-300 space-y-8">
            <div>
              ...
              <p className="text-[#7389ca] mt-2 max-w-xl text-xs font-mono">
                [Player profiles active]. Search, verify, or disable users
                participating in Quiniela-Mundial-2026.
              </p>
            </div>
              ...
              <table className="w-full text-left font-mono text-xs text-[#7389ca]">
                ...
                <tbody className="divide-y divide-[#7389ca]/20 bg-[#000c2e]">
                  {users.map((usr) => (
                    <tr
                      key={usr.id}
                      className="hover:bg-primary-container/10"
                    >
                      ...
                      <td className="p-4 text-right">
                        <button
                          onClick={() =>
                            handleToggleUser(usr.id, usr.name)
                          }
        ...
        {activeTab === "predictions" && (
          <div className="animate-in fade-in duration-300 space-y-8">
            <div>
              ...
              <p className="text-[#7389ca] mt-2 max-w-xl text-xs font-mono">
                [Telemetry logs]. Analyzing predictions data structures.
                1,240 active ballots uploaded.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="terminal-border bg-primary-container/20 p-6 space-y-4">
                <h3 className="font-headline font-bold text-white uppercase text-sm border-b border-[#7389ca]/30 pb-2">
                  Global Predictions Distribution
                </h3>
        ...
        {activeTab === "logs" && (
          <div className="animate-in fade-in duration-300 space-y-8">
            <div>
              ...
              <p className="text-[#7389ca] mt-2 max-w-xl text-xs font-mono">
                [Raw logs terminal]. Event telemetry for actions taken in
                this session.
              </p>
            </div>
              ...
              <div className="font-mono text-xs text-[#7389ca] space-y-2 h-[400px] overflow-y-auto bg-[#000c2e] p-4 border border-[#7389ca]/30">
                {systemLogs.map((log, idx) => (
                  <p key={idx}>
                    ...
                    <span
                      className={
                        log.msg.includes("[MATCH_UPDATE]")
                          ? "text-green-400"
                          : log.msg.includes("[SYS_RESET]")
