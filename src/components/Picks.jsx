...
export default function Picks({ user, onUpdateStats }) {
  ...
  const userRank = useMemo(() => {
    ...
    const ranked = Object.entries(userStats).sort(
      (a, b) =>
        b[1].points - a[1].points ||
        b[1].correctScores - a[1].correctScores,
    );
  ...
  // Calculations
  const savedCount = Object.keys(savedMatches).filter(
    (k) => savedMatches[k],
  ).length;
  const totalPicks = matches ? matches.length : 0;
  const remainingPicks = Math.max(0, totalPicks - savedCount);
  const completionPercent =
    totalPicks > 0 ? Math.round((savedCount / totalPicks) * 100) : 0;
  ...
  return (
    <div className="max-w-container-max mx-auto px-margin-mobile md:px-gutter pt-stack-md md:pt-stack-lg min-h-screen pb-32">
      ...
      <div className="mb-stack-md flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="text-headline-md md:text-headline-lg font-headline-lg text-on-background mb-2">
            Mis Picks
          </h1>
          <p className="text-body-md text-on-surface-variant">
            Envía tus predicciones para los próximos partidos.
          </p>
        </div>
              ...
              .map(([group, groupMatches]) => (
                <section key={group}>
                  <div className="flex items-center gap-4 mb-stack-sm">
                    <span className="text-label-sm font-label-sm bg-primary text-on-primary px-3 py-1 rounded uppercase">
                      GROUP {group}
                    </span>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {groupMatches.map((match) => {
                      const matchId =
                        match.id || `${match.home}_${match.away}`;
                      const isSaved = savedMatches[matchId];
                      const isSavedInApi = apiPicks?.some(
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            <div className="flex flex-1 items-center gap-4 w-full md:w-auto">
                              <div className="w-12 h-12 flex-shrink-0 shadow-sm rounded-full overflow-hidden border border-outline-variant/30">
                                <img
                                  src={`https://flagcdn.com/${getCountryCode(match.home)}.svg`}
                                  alt={`${match.home} flag`}
                            ...
                            <div
                              className={`flex items-center gap-4 bg-surface-container-low p-2 rounded-xl border transition-all ${
                                (predictions[`${matchId}_home`] !== "" &&
                                  predictions[`${matchId}_home`] !==
                                    undefined) ||
                                (predictions[`${matchId}_away`] !== "" &&
                                  predictions[`${matchId}_away`] !==
                                    undefined)
                            ...
                            >
                              ...
                              />
                              <span className="text-on-surface-variant font-bold text-headline-md">
                                VS
                              </span>
                              <input
                                type="number"
                            ...
                            <div className="flex flex-1 items-center justify-end gap-4 w-full md:w-auto">
                              ...
                              <div className="w-12 h-12 flex-shrink-0 shadow-sm rounded-full overflow-hidden border border-outline-variant/30">
                                <img
                                  src={`https://flagcdn.com/${getCountryCode(match.away)}.svg`}
                                  alt={`${match.away} flag`}
                                  className="w-full h-full object-cover"
                                />
                              </div>
                          ...
                          <div className="mt-4 flex flex-col items-center justify-center gap-1">
                            ...
                            {match.match_date && (
                              <span
                                className={`text-xs font-bold tracking-widest uppercase ${new Date(match.match_date) - now <= 0 ? "text-red-500" : "text-orange-500 animate-pulse"}`}
                              >
                          ...
                          {isSaved && (
                            <div className="mt-3 flex justify-center">
                              <span className="bg-secondary text-on-secondary px-3 py-0.5 rounded-full text-label-sm font-label-sm animate-in fade-in zoom-in-95 duration-200">
                                SAVED
                              </span>
                            </div>
                          )}
        ...
        <div className="lg:col-span-4 space-y-gutter">
          ...
          <div className="bg-primary text-on-primary p-6 rounded-xl shadow-lg relative overflow-hidden border border-primary-fixed/20">
            <div className="relative z-10">
              <h3 className="text-headline-md font-headline-md mb-4 text-on-primary">
                Your Ranking
              </h3>
              <div className="flex items-end gap-2 mb-6">
                ...
                <span className="text-body-md opacity-80 pb-2">
                  of {userRank?.total ?? "---"} users
                </span>
              </div>
              <div className="space-y-4">
                ...
                <p className="text-label-sm font-label-sm opacity-80 uppercase tracking-widest text-white">
                  Picks Completed: {savedCount} / {totalPicks} (
                  {completionPercent}%)
                </p>
              </div>
            </div>
            <div className="absolute -right-4 -bottom-4 opacity-10">
              <span className="material-symbols-outlined text-[120px] text-tertiary-fixed">
                military_tech
              </span>
            </div>
          </div>
          ...
          <div className="bg-surface-container-high border border-outline-variant p-6 rounded-xl">
            <h4 className="text-label-sm font-label-sm text-on-surface-variant mb-4 uppercase tracking-wider">
              Status Legend
            </h4>
      ...
      >
        ...
        <span className="text-body-md font-bold uppercase tracking-tight">
          Save All Picks
        </span>
      </button>
    </div>
